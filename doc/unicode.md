...
，由先前对 `iconv_open(3)` 的调用创建；该转换描述符定义了 `iconv()` 在转换过程中使用的字符编码。`inbuf` 参数是一个变量的地址，该变量指向输入序列的第一个字符；`inbytesleft` 表示该缓冲区中的字节数。`outbuf` 参数是一个变量的地址，该变量指向输出缓冲区中可用的第一个字节；`outbytesleft` 表示输出缓冲区中可用的字节数。
